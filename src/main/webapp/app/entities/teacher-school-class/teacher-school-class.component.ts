...
@Component({
  selector: 'jhi-teacher-school-class',
  templateUrl: './teacher-school-class.component.html'
})
export class TeacherSchoolClassComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.teacherService
      ...
      .subscribe(
        (res: HttpResponse<ITeacherSchoolClass[]>) => this.paginateTeachers(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/teacher-school-class'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: ITeacherSchoolClass) {
    return item.id;
  }
  ...
  protected paginateTeachers(data: ITeacherSchoolClass[], headers: HttpHeaders) {
