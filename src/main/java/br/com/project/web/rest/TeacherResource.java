...
/**
 * REST controller for managing {@link br.com.project.domain.Teacher}.
 */
@RestController
@RequestMapping("/api")
public class TeacherResource {
     ...
     */
    @GetMapping("/teachers")
    public ResponseEntity<List<TeacherDTO>> getAllTeachers(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("history-is-null".equals(filter)) {
            log.debug("REST request to get all Teachers where history is null");
            return new ResponseEntity<>(teacherService.findAllWhereHistoryIsNull(),
                    HttpStatus.OK);
        }
