...
/**
 * REST controller for managing {@link br.com.project.domain.Subject}.
 */
@RestController
@RequestMapping("/api")
public class SubjectResource {
     ...
     */
    @GetMapping("/subjects")
    public ResponseEntity<List<SubjectDTO>> getAllSubjects(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("history-is-null".equals(filter)) {
            log.debug("REST request to get all Subjects where history is null");
            return new ResponseEntity<>(subjectService.findAllWhereHistoryIsNull(),
                    HttpStatus.OK);
        }
